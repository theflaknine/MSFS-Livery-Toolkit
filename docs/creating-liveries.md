...
The toolkit classifies each texture using the official SDK **sidecar metadata** from the base aircraft — never by filename. So a file named `..._ALBEDO.PNG` that actually carries alpha transparency is correctly treated as a Decal/Transparent map.

| Type | What it is |
|---|---|
| **Albedo** | The core colour and paint map. |
| **Composite** | A packed image of Ambient Occlusion, Roughness, and Metallic. |
| **Normal** | Surface detail. |
| **Emissive** | Self-illuminated areas — backlit windows, instruments, strobes. |
| **Decal / Transparent** | An overlay layer that relies on an alpha channel. |
...
- **Generate placeholders** — blank, correctly-sized canvases labelled with the filename and resolution.
...
{: .important }
> **Your artwork is protected.** The tool never overwrites an existing image file in your workspace — neither a placeholder nor an extracted image will clobber artwork you've already put there. Once you replace a placeholder with your own painting, it is safe.
